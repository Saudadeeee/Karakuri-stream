...
- **`shader_budget.gd`** (`class_name ShaderBudget`) — **ĐỌC TRƯỚC KHI TẠO BẤT KỲ MATERIAL NÀO TRÊN NHÁNH NÀY.** `gl_compatibility` biên dịch **một chương trình cho mỗi TỔ HỢP CỜ TÍNH NĂNG** của material, đồng bộ, ngay lần đầu có thứ mang tổ hợp đó được vẽ. **Màu albedo là miễn phí** (trăm màu chung một chương trình); **cờ thì không**. Trên desktop vài ms không ai để ý; qua ANGLE ra D3D11/Metal mỗi chương trình tốn **nhiều giây** — đo thật trên RTX 3060: chương trình lit đầu tiên 13.8 giây, mỗi `.gdshader` ~4 giây. Nhánh này chốt **6 tổ hợp**: lit đục, lit phát sáng, lit vertex-colour, unshaded alpha, unshaded cộng, billboard hạt. Thứ đến từ bên ngoài (một `.glb` bị exporter đánh dấu 2 mặt) được nắn về tập đó trên đường vào, qua `MeshFit.matte` — lượt duyệt mọi model nhập vốn đã đi qua. **BẪY ĐÃ CẮN:** đặt `specular_mode = SPECULAR_DISABLED` cho riêng model nhập chỉ **dời** vết nứt từ `cull_mode` sang `specular_mode`, đếm lại vẫn 9. Giết specular bằng **cường độ** (uniform, miễn phí), không bằng **mode** (cờ). Cờ khác uniform — đó là toàn bộ nội dung của file này.
- **`boot_warmup.gd` / `scenes/boot.tscn`** — cảnh chạy ĐẦU TIÊN, tồn tại CHỈ vì trình duyệt. Menu vẽ đảo + trời + cảnh vật + nước cùng lúc nên frame đầu của nó nợ driver **mọi** chương trình game có: đo được **~41 giây đơ cứng**, không spinner, không tiến trình — người ta đọc là "hỏng rồi" và đóng tab. Không tránh được (compat renderer không có đường async) nhưng **tiêu được ở chỗ nhìn thấy**: vẽ một thứ mỗi frame → driver dịch một chương trình mỗi frame, giữa mỗi cái trình duyệt lấy lại main thread, thanh tiến trình chạy. Đo Chrome thật (ANGLE→D3D11, RTX 3060): **lần đầu 42.3s tới menu, mọi lần sau 3.8s** — Chrome giữ program binary trong profile nên **biên dịch là chi phí một lần trên mỗi máy**. **HAI BẪY ĐÃ CẮN:** (1) đèn hâm nóng bật shadow → biên dịch cả biến thể depth-pass mà hồ sơ web không dùng, 65s → 37s chỉ nhờ một dòng; (2) hâm trong `SubViewport` → chương trình gắn với render target nó được dịch cho, hâm ở chỗ khác kích thước/định dạng có nguy cơ trả tiền hai lần. Giờ hâm ở viewport chính, nấp sau tấm nền đục. Chi tiết + việc còn lại: **[web.md](web.md)**.
- **`tools/floor.gd`** (KHÔNG export) — trả lời "thời gian khởi động web đi đâu". Dựng một scene rồi thêm TỪNG THỨ một, in chi phí mỗi bước, để tách **sàn của engine** khỏi thứ do game chọn. Kết quả đảo ngược mọi phỏng đoán: 2D-only 180ms, **chỉ thêm `Camera3D` = 26 giây** (chưa vẽ gì), trời + sương + grade + đèn + hình khối cộng lại ~4 giây, **emissive một mình 8.6 giây**, hộp lit đầu tiên 19ms, hộp thứ hai khác màu 21ms (màu miễn phí, đúng như hứa). **Cắt hình khối không bao giờ giúp được gì** — bài học đắt nhất của nhánh này, và không có cách nào đoán ra nếu không đo.
- **`shaders/surface.gdshader`** — MỘT program cho cả gỗ, nước tĩnh và dòng chảy, chuyển bằng uniform `surface_mode`. Ba shader = ba lần biên dịch (7.8 + 7.8 + 7.4 giây qua ANGLE); một shader có nhánh = một lần. Nhánh là *dynamically uniform* nên GPU chỉ đi một phía cho cả draw, không tốn gì lúc chạy. Gỗ và nước vốn chỉ khác `diffuse_burley`/`diffuse_lambert`; dòng chảy bị tách vì `blend_mix` (render_mode = compile-time, uniform không đổi được) nhưng alpha của nó là 0.98 và nước trong game vốn đục có chủ ý, nên cho nó đục thật là nhập được. **Thêm shader mới = thêm vài giây vào lần vào đầu tiên của mọi người chơi.**
- **HẠT DÙNG `CPUParticles3D`, KHÔNG dùng `GPUParticles3D`.** `ParticleProcessMaterial` là một shader riêng — 7.4 giây biên dịch nguội, chỉ để mô phỏng vài chục cái quad. Bản web vốn đã giảm nửa số hạt. Đổi sang CPU thì chi phí còn 0.5 giây.
- **`tools/perf.gd`** (KHÔNG export) — đo thời gian khung bằng renderer THẬT, ép hồ sơ LITE của web. **Phải tắt vsync**, không thì mọi cấu hình đều báo đúng chu kỳ màn hình (6.95 ms trên panel 144 Hz) và số đo vô nghĩa. Ngưỡng spike là **tuyệt đối (>16.7 ms)**, không phải "gấp đôi trung vị" — cái sau phụ thuộc quy mô nên gắn cờ cả khung 3 ms bình thường trên scene chạy 1.2 ms.
  **`churn=1` chỉ bẩn isosurface — nó KHÔNG đặt hay xoá khối.** Nên suốt thời gian dài harness chỉ đo một khu vườn đã dựng xong đang được NGẮM, chưa bao giờ đo hành động ĐẶT, tức là việc duy nhất người chơi làm; chi phí tỉ lệ với quy mô thị trấn vô hình ở đây đúng vì lý do đó. Giờ `place=1` (mặc định) thật sự đặt rồi xoá một khối trong cửa sổ đo, `ptype=N` chọn loại khối.
  `godot --path . --rendering-driver opengl3 --resolution 1280x720 tools/perf.tscn -- scene=res://scenes/main.tscn lite=1 blocks=80 churn=1`
