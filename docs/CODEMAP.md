...
tests/
  ...
  blocks/
    ...
    house_block.tscn     **NHÀ (Townscaper-style)** — người chơi đặt 1 Ô NHÀ, toà nhà tự lắp. Logic "cái gì mọc ở đâu" nằm HẾT ở `HouseShape` (data class), `house_block.gd` chỉ dựng hình — đúng cặp `PipeRouting`/`pipe_block`. Tường CHỈ mọc ở mặt không có nhà kề (tường chung biến mất → 1 dãy đọc thành MỘT toà nhà dài, không phải 3 cái lều). **Mái KHÔNG phải hình của từng ô** — là 1 TRƯỜNG ĐỘ CAO của cả toà nhà (xem `house_shape.gd`), mỗi ô chỉ dựng miếng vá 3×3 sample của mình; ô lơ lửng mọc **cột chống** hoặc **thanh chống nghiêng** (không được phép lơ lửng); cửa/ống khói mỗi toà nhà đúng 1 cái, lấy theo ô đầu/cuối thứ tự lexicographic của component; ban công ở tầng trên. Tất cả DETERMINISTIC (`HouseShape._h()` hash ô, KHÔNG randf) → reload ra đúng ngôi nhà cũ. Đục 1 ô giữa dãy → 2 đầu tự mọc tường + đầu hồi. **CHI PHÍ SỬA THỊ TRẤN** (đo bằng `tools/perf.gd houses=48`, cùng máy): đặt 1 ô nhà từng tốn **frame 101ms**, giờ **12ms, 0/420 frame quá 16ms**. Ba tầng chặn, theo thứ tự:
    (1) `_digest` BỎ `building_size` — đó là số ô của toà nhà, hình học KHÔNG hề đọc nó, mà thêm 1 ô vào toà 48 ô thì cả 48 digest đổi → 48 mesh bị vứt đi dựng lại y hệt (76ms/click). Mọi thứ SUY RA từ nó (decor tier → bồn hoa/mái hiên/vườn mái) đã nằm trong digest dưới dạng boolean kết quả.
    (2) `_digest` chẻ đôi: nửa TOÀ NHÀ (rẻ, tra cache component) + nửa HÀNG XÓM (~40 query, phần đắt). Nửa hàng xóm chỉ tính lại khi `HouseShape.touches_locally()` — sửa đổi rơi trong bán kính `REACH` — hoặc khi nửa toà nhà đổi (decor tier nuôi trim từng mặt).
    (3) NGÂN SÁCH THỜI GIAN 3ms/frame dùng chung cả thị trấn (`REBUILD_BUDGET_US`): ô vượt ngân sách giữ cờ dirty, dựng frame sau. Ô người chơi vừa đặt KHÔNG bị hoãn — nó dựng đồng bộ từ setter `grid_cell`. Hệ quả: hình học hội tụ sau vài frame, nên test phải `await _settle_houses()` chứ không đếm frame cố định.
    **MỘT draw call mỗi ô** (`MeshBatch.build_merged`): trước đây mỗi MÀU là một surface + một `StandardMaterial3D` → 4-5 draw call/ô, thị trấn 120 nhà = **4098 draw call**. Giờ màu đi theo VERTEX trên một material dùng chung cho cả thị trấn → **1391**. Chỉ màu cần material riêng (kính cửa sổ phát sáng ban đêm) mới giữ surface riêng. Màu ghi thẳng, KHÔNG `srgb_to_linear()` — renderer compatibility đối xử vertex color y như albedo, convert thêm là ra nhà đen sì.
    Gộp mesh qua `MeshBatch` (xem dưới). Nhà giờ chia 2 file: `house_block.gd` quyết định ô này LÀ gì (query HouseShape, digest, ngân sách dựng), `house_roof.gd` (`class_name HouseRoof`) dựng mọi thứ nằm TRÊN nóc — trường độ cao mái, chóp, sân thượng, cửa sổ mái, vườn mái, sân trong, dây cờ, ống khói. Toàn hàm static, nhận `paint` (bảng màu đã nhuộm theme) nên không biết gì về theme.
...
| 4 | `StreamManager` | `autoload/stream_manager.gd` | **CƠ CHẾ CHÍNH (karakuri)**: mỗi khối SOURCE phun dòng thẳng xuống; trace ô-theo-ô: qua PIPE/PIPE_BEND thì vào 1 port ra port kia (đọc `state["ports"]`), else rơi theo trọng lực tới khi trúng khối. `_trace()` gom `_segments` (dựng cylinder nước cyan) + `_impacts` (ô đích) + `_driven_gears`. Ô đích phát âm theo loại (gỗ/pipe→wood_hit, bell→chime+firefly, gear→clack+quay) mỗi `IMPACT_INTERVAL=0.55s` + splash. Rebuild throttled khi lưới đổi. **MẶT CỎ KHÔNG NẰM TRONG LƯỚI**: đảo là MESH do `IslandBuilder` đắp, không phải khối, nên `GridManager` không có gì ở y<0 để chặn dòng rơi. Trước đây nước rót xuống nền trống **rơi XUYÊN qua đảo tới y=-18** — 18 segment vô hình dưới thế giới, không splash, không impact, nên `is_playing()` mãi false và 1 cái vòi cắm trên cỏ là câm tịt. Giờ `_trace` chặn ở `nxt.y < 0` khi còn trong đĩa đảo (`MAP_RADIUS+1`), dựng segment tới ĐÚNG y=0 bằng `_add_seg_world`, và `_impact(WATER)` (splash, KHÔNG gõ — đất không kêu cồm cộp). Ngoài vành thì vẫn rơi vào biển mây như cũ. Ô impact lấy y=-1 CÓ CHỦ Ý: `_play_impact` splash ở MẶT TRÊN của ô đó, tức đúng y=0. |
...
**3 bug VỊ TRÍ chỉ lộ ra khi RENDER và NHÌN** (logic đúng hết, số lượng đúng hết, chỉ sai chỗ đứng) — đã ghim bằng `_sec_wildlife_placement`:
 • `_top_of()` trả về MẶT TRÊN của ô, nhưng ô NHÀ còn có MÁI cao hơn mặt đó → mèo/chim lún trong mái. Phải cộng `HouseShape.roof_top_height()`.
 • Vịt trước đây bơi quanh **trọng tâm** hồ — hồ hình L thì trọng tâm nằm TRÊN CẠN. Giờ nhảy giữa các Ô NƯỚC thật, mọi waypoint đều là nước theo cấu trúc. `_pond_set` (Dictionary) cho tra cứu O(1) vì vịt test mỗi frame.
 • Chim chọn chỗ đậu ngẫu nhiên → 2 con trùng ô, render thành 1 con thừa cánh. `_free_perch()` loại ô đã có chim. |
| 13a | — | **GỘP SURFACE CHO PROP** | `MeshFit.bake()` — model glTF ra mỗi MÀU một surface + một material (3-4/khối), và mỗi surface là 1 draw call, rồi nhân tiếp cho pass bóng đổ: 800 khối = **13.498 draw call, 61 fps**. Gộp thành 1 surface màu-theo-vertex → **6.063 draw call, 87 fps, 3/420 frame quá 16ms**. Cache theo (mesh nguồn + đúng dãy màu) nên mọi bản sao cùng biến thể dùng chung 1 ArrayMesh. **PHẢI GIỮ MẢNG INDEX**: surface nhập từ glTF là index buffer trên một danh sách đỉnh dùng chung — nối mảng đỉnh mà bỏ index thì mọi model vỡ thành nan quạt (đã dính đúng lỗi này, `_sec_batching_and_touch` ghim lại). Chạy sau `tint`/`recolor_foliage`; `tint` gặp mesh đã bake thì cảnh báo to chứ không im lặng làm sai. |
| 13b | — | **CẢM ỨNG (Android/web mobile)** | `scripts/ui/touch_controls.gd` — 2 nút Erase + Undo góc dưới phải, CHỈ hiện khi `DisplayServer.is_touchscreen_available()` hoặc build mobile (ép bằng env `KARAKURI_FORCE_TOUCH` để chụp thử trên desktop). Lý do tồn tại: chạm = chuột TRÁI (Godot emulate), **không có cử chỉ nào sinh chuột phải** → trên APK trước đây đặt được mà không xoá được, undo lại gán Ctrl+Z. Không dùng long-press: emulate đã bắn "đặt" ngay lúc ngón chạm xuống, nên long-press buộc phải đặt rồi undo — thấy rõ và sai khi ngón trượt. `placement_controller.erase_mode` bật thì click trái = gỡ, và ghost bị ẩn (xem trước một khối sẽ không được đặt là nói dối). |
| 14 | `DisplaySettings` | `autoload/display_settings.gd` | Fullscreen / kích thước cửa sổ / vsync, lưu trong `settings.cfg` mục `[display]`, áp TRƯỚC frame đầu. **F11** toggle fullscreen ở mọi scene. Kéo cạnh cửa sổ → nhớ kích thước mới (debounce 0.6s). Bỏ qua kích thước đã lưu khi cửa sổ lúc boot KHÁC mặc định project — đó là dấu hiệu có `--resolution` trên dòng lệnh (engine flag bị strip khỏi `OS.get_cmdline_args()`, không test trực tiếp được), nếu không harness `tools/shoot.gd`/`perf.gd` bị resize lén. |
| 14b | `I18n` | `autoload/i18n.gd` | **Tiếng Việt, không cần bước build.** KHOÁ CHÍNH LÀ CHUỖI TIẾNG ANH: Control tự dịch `text` qua TranslationServer, nên mọi nhãn/nút ăn bản dịch ngay khi đăng ký theo đúng chuỗi tiếng Anh của nó — không rải `tr()` khắp UI, không CSV phải re-import, và chuỗi chưa dịch thì rơi về tiếng Anh chứ không hiện `UI_MENU_PLAY`. Chỉ chuỗi GHÉP LÚC CHẠY (`"Saved to garden %d"`) mới phải `tr()` tại chỗ gọi rồi mới `%`. Ngôn ngữ lưu ở `settings.cfg [ui] locale`; máy hệ Việt tự mở tiếng Việt. **Font**: pixel + Fredoka đều dừng ở Latin-1 → chữ có dấu ra ô vuông; `ui/karakuri_theme.tres` xếp `Nunito.ttf` (OFL) làm mắt xích CUỐI trong `fallbacks` — phải để ở MẢNG ĐÓ, Godot không đi tiếp vào fallback của fallback. |
| 15 | `DiscoveryLog` | `autoload/discovery_log.gd` | **Sổ khám phá** — 12 điều thị trấn tự làm (spire/terrace/courtyard/bunting/arch/dormer/roof garden/stilts + 4 loài vật). Poll 0.7s, chỉ quét khi lưới bẩn, và BỎ hẳn rule đã tìm ra → vườn cũ không tốn gì. Ghi vào `settings.cfg` mục `[discovered]` chứ KHÔNG vào save: thứ người chơi đã THẤY thuộc về người chơi, xoá build không được xoá nó. `main_scene` hiện thẻ 1 lần (queue `take_pending()`, không chồng thẻ), `pause_menu` có nút Journal liệt kê — mục chưa tìm ra hiện `? ? ?` vì viết tên ra là phát công thức. |
...
- **`house_shape.gd`** (`class_name HouseShape`) — TOÀN BỘ luật kiến trúc của nhà, tách khỏi phần dựng hình.
  ...
  **NGUYÊN TẮC (đã sai 4 lần, mỗi lần một dạng): thứ gì đọc ra MỘT BỀ MẶT thì phải quyết định trên CẢ MẢNG, không phải từng ô.** Mái → trường độ cao trên cả nền nhà. Sân thượng → flood cả mảng mái. Chóp → `roof_patch(cell).size() == 1` (luật cũ đếm hàng xóm CỦA CHÍNH Ô và cho phép 1 → đỉnh rộng 2 ô thì CẢ HAI đều thoả, mọc 2 chóp cạnh nhau). Quyết định theo ô luôn tạo vá chằng đúng chỗ hai ô gặp nhau. `roof_patch()` là helper dùng chung cho mọi câu hỏi kiểu này.
  **SÂN THƯỢNG thuộc về MẢNG MÁI, không thuộc về ô.** Quyết định theo từng ô là **đúng cái sai** mà mái-theo-từng-ô đã mắc, chỉ ở tầng cao hơn: trên khối 2×2, ô chạm tháp thành sàn phẳng còn ô phía sau giữ mái dốc → đỉnh toà nhà ra vá chằng vá đụp, nhìn rõ là **không gộp**. Giờ flood cả mảng mái liền nhau ở cùng độ cao: **bất kỳ ô nào** trong mảng đứng dưới bóng phần cao hơn thì **CẢ MẢNG** thành một sàn liền. Cache theo `GridManager.version` như `_component`.
  **`_face_size` vs `_slab` — BẪY ĐÃ CẮN.** `_face_size(side, span, thick)` nhét `span` vào **CẢ trục đứng LẪN trục ngang** → chỉ đúng cho thứ phủ kín cả mặt, tức BỨC TƯỜNG. Mọi thứ THẤP (bệ, đai, lan can) phải dùng `_slab(out, rộng, cao)` nhận riêng hai chiều. Lan can sân thượng dùng nhầm `_face_size` → "rộng 1.04, dày 0.09" thành **tấm cao 1.04**, và sân thượng render ra thành **vòng tường không mái đứng quanh một ô TRỐNG** — người chơi đặt được khối vào đó và khi đặt xong thì hết lỗi (vì ô thôi làm sân thượng). Không test logic nào bắt được: luật vẫn đúng, chỉ hình sai. Đã ghim bằng `_sec_terrace_is_flat` đo AABB mesh THẬT.
  ...
  **HAI BẪY ĐÃ CẮN, ĐỪNG LẶP LẠI:**
  1. `_component()` **KHÔNG được gieo hạt từ ô không phải nhà.** `queue_free()` là hoãn, nên ô vừa bị xoá VẪN nhận `block_removed` rồi tự rebuild lần cuối — flood gieo hạt vô điều kiện sẽ đi từ ô CHẾT ngược vào toà nhà cũ, tự tính mình là thành viên, rồi **ghi kết quả sai đó vào cache DÙNG CHUNG cho mọi ô còn sống**. Xoá đế tháp mà đỉnh vẫn tưởng mình 4 tầng và giữ chóp; nhà ra "mất mái" sau khi xoá. Ô chết trả về `size: 0` và **KHÔNG cache**.
  2. **Thông báo rebuild phải tới MỌI ô nhà, không chỉ hàng/cột kề.** Màu, cửa, ống khói, chóp, sân thượng, sân trong đều quyết định từ CẢ TOÀ NHÀ. Luật cũ chỉ báo cho ô cùng hàng/cột cùng y → xoá đế tháp không báo cho đỉnh, gãy một nhánh chữ L không báo cho nhánh kia. Giờ mọi thay đổi liên quan nhà đều đặt `_dirty` và rebuild **gộp trong `_process`, tối đa 1 lần/frame/ô** — làm đồng bộ sẽ rebuild cả thị trấn mỗi signal.
  3. **"Báo cho mọi ô" KHÔNG có nghĩa là "báo cho cả bản đồ", và cũng KHÔNG có nghĩa là "dựng lại".** Sửa (2) đã đi quá đà thành `HouseShape.is_house(cell)` không giới hạn khoảng cách: đặt 1 nhà thì MỌI nhà trên đảo rebuild. Đo được **~10 ms mỗi nhà đã có** — 33 ms trên đảo trống, **818 ms trong thị trấn 80 nhà**; chính là "tụt FPS 80-90 xuống 30-40". Hai lớp chặn, cả hai đều cần:
     • **Phạm vi** — `HouseShape.affects(cell, changed)`: hộp bao của component nới thêm `REACH = ARCH_SPAN + 1`, cộng cột phía dưới cho cột chống. Ngoài hộp đó thì không thể liên quan. Lưu ý `lo`/`hi` của `_component()` là cực trị **lexicographic** (dùng để chọn cửa), KHÔNG phải hộp bao — hộp bao là `bmin`/`bmax`.
     • **Digest** — `house_block._digest(ctx)`: gói MỌI câu trả lời `HouseShape` mà bản dựng dùng thành một chuỗi; giống hệt lần trước thì giữ nguyên mesh. Cần vì khi cả thị trấn là MỘT toà nhà 80 ô thì `affects()` đúng khi trả true cho cả 80, nhưng chỉ 2 ô thật sự đổi hình. **Thêm truy vấn `HouseShape` mới vào bản dựng thì PHẢI thêm vào `_digest`**, không thì ô giữ hình cũ. Khung tệ nhất 161 ms → 42 ms ở 80 nhà.
     Lưới an toàn là `_sec_no_stale_geometry`: sửa-rồi-đo so với dựng-mới-từ-đầu phải **khớp tuyệt đối**. Nó đã được kiểm chứng là bắt được lỗi (siết `affects` xuống bán kính 1 → đỏ ngay).
     Phần còn lại của cú click (~21 ms, **giống hệt nhau khi đặt và khi xoá**, không phụ thuộc số nhà) KHÔNG phải của nhà — đó là `VoxelSurfaceManager` giải lại isosurface khi lưới đổi thật.
...
- **`tools/perf.gd`** (KHÔNG export) — đo thời gian khung bằng renderer THẬT, ép hồ sơ LITE của web. **Phải tắt vsync**, không thì mọi cấu hình đều báo đúng chu kỳ màn hình (6.95 ms trên panel 144 Hz) và số đo vô nghĩa. Ngưỡng spike là **tuyệt đối (>16.7 ms)**, không phải "gấp đôi trung vị" — cái sau phụ thuộc quy mô nên gắn cờ cả khung 3 ms bình thường trên scene chạy 1.2 ms.
  **`churn=1` chỉ bẩn isosurface — nó KHÔNG đặt hay xoá khối.** Nên suốt thời gian dài harness chỉ đo một khu vườn đã dựng xong đang được NGẮM, chưa bao giờ đo hành động ĐẶT, tức là việc duy nhất người chơi làm; chi phí tỉ lệ với quy mô thị trấn vô hình ở đây đúng vì lý do đó. Giờ `place=1` (mặc định) thật sự đặt rồi xoá một khối trong cửa sổ đo, `ptype=N` chọn loại khối.
  `godot --path . --rendering-driver opengl3 --resolution 1280x720 tools/perf.tscn -- scene=res://scenes/main.tscn lite=1 houses=80 churn=1`
...
- **BA SLOT** (`SLOT_COUNT=3`): mỗi vườn là file riêng `user://garden_<i>.json` + `.bak.json` + `.json.tmp`. KHÔNG có file index — slot nào tồn tại là do file có tồn tại hay không, nên không thể lệch với đĩa. Slot đang chơi lưu ở `settings.cfg` mục `[save] slot`; `save_game()`/`load_game()`/`has_save()` không truyền tham số = slot đó. `slot_info(i)` đọc HEADER để mô tả slot (số khối, map, thời điểm lưu) mà không phải dựng lại cả vườn. Bản cũ 1 slot (`user://save_data.json`) được `_migrate_legacy()` **đổi tên** (không copy — copy thì chạy 2 lần là hồi sinh vườn người chơi đã xoá) sang garden 1 ở lần chạy đầu.
- Định dạng **v2**: `{"v":2,"saved_at":<unix>,"theme":<int>,"count":<int>,"blocks":[...]}`. Header có để menu mô tả slot mà không phải load. **v1 (mảng trần) vẫn đọc được** — `_read_entries` thấy Dictionary thì lấy `["blocks"]`, thấy Array thì dùng luôn; save của người chơi bản cũ không được phép chết.
- Mỗi phần tử block: `{"x":int,"y":int,"z":int,"type":int}` (`type` là giá trị số của `BlockData.Type`). Gear thêm `"axis":[ax,ay,az]` (trục đặt); khi load, entry có axis + type GEAR → set `block.state["axis"]` + gọi `instance.apply_axis()`. Save cũ thiếu `axis` → gear nằm ngang mặc định (+Y) — backward-compatible.
- `save_game(slot=-1)`: `GridManager.get_all_cells()` (KHÔNG lọc theo type — quét toàn bộ `_blocks`, chấp nhận được vì chỉ chạy lúc bấm Lưu, không phải mỗi frame) → serialize → ghi file.
- `load_game(slot=-1)`: đọc file → `GridManager.clear_all()` (xoá sạch lưới hiện tại trước, không merge với build cũ) → với mỗi entry: `BlockFactory.instantiate(type)` → add làm con của `get_tree().current_scene` (KHÔNG phải con của `SaveManager` — `SaveManager` là autoload, cha của nó là `/root`, không phải "Main") → `GridManager.set_block(...)`. Trả `false` nếu file không tồn tại hoặc JSON hỏng (`data == null` hoặc không phải `Array`) — gọi ở nơi khác PHẢI check giá trị trả về, đừng giả định luôn thành công.
