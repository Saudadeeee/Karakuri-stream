...
## PHẦN 63: Phá hủy prop môi trường khi đè block

User: đặt block đè prop (cây/bụi/đá) → prop bị phá hủy có hiệu ứng.

- `SceneryManager`: `_prop_cells` map cell→{root,kind} (chỉ prop RING trên đảo, không núi backdrop); nghe `GridManager.block_placed` → nếu đặt trúng cell prop → `_shatter`: topple (xoay z + scale→0 TRANS_BACK 0.28s) + `burst` puff hạt màu theo loại (lá xanh / đá xám / gỗ) + knock nhẹ, rồi free.
- `burst()` public → `DecorManager` tái dùng: hoa/mầm mọc trên gỗ bị block đè cũng poof (thay vì biến mất câm).
...
## PHẦN 73 (main): MỘT REGISTRY CHO BLOCK + DỌN DOCS + SỔ TAY MAINTAIN

User: "cải tiến + hoàn thiện, sắp xếp lại hợp lý, và một file doc để người low-tech tự maintain được".

### Điểm đau tìm ra: thêm 1 block = sửa 5 file / 8 chỗ
`block_data.gd` (enum) → `block_factory.gd` (const preload **và** SCENES_BY_TYPE = 2 chỗ) → `material_ui.gd` (HINTS **và** ENTRIES = 2 chỗ) → `placement_controller.gd` (chuỗi if keycode) → scene + script. Sót bất kỳ chỗ nào ra block "tồn tại một nửa": đặt được nhưng không có icon, hoặc có icon mà không phím tắt (GATE đang đúng tình trạng này — nằm trên hotbar nhưng không có phím).

### Fix: `scripts/data/block_catalog.gd` (`class_name BlockCatalog`)
Một danh sách `ALL`, mỗi block một entry: `type / scene(preload) / hotbar / key / hint`. Thứ tự entry = thứ tự icon.
- `block_factory.instantiate()` đọc `scene` từ catalog — file factory từ 56 dòng còn 19, hết double-bookkeeping.
- `material_ui`: `ENTRIES` = `palette_types()`, hint = `BlockCatalog.hint()` — xoá 2 bảng cứng.
- `placement_controller`: chuỗi 13 `elif keycode` → `BlockCatalog.type_for_key()`. Tiện thể GATE có phím G.
- `save_manager` validate type qua catalog thay vì `SCENES_BY_TYPE`.
→ **Thêm block giờ = 3 bước: enum, scene, 1 entry.**

### Test mới `_sec_catalog` (chạy đầu tiên, không cần scene)
Bắt đúng lỗi catalog sinh ra để phòng: type trùng, thiếu scene/hint, **2 block giành 1 phím**, phím trỏ nhầm type, và **mọi giá trị trong `BlockData.Type` phải có entry + instantiate được**. Đây là lưới an toàn để người low-tech thêm block mà không sợ sót.

### Dọn docs (merge PHẦN 70 để lại bãi chiến trường)
- **2 file plan.md** (root 49 dòng = PHẦN 70-72, `docs/` 1090 dòng = 1-69) → gộp về `docs/plan.md`, giờ 67 mục liền mạch.
- `AUDIO.md` → `docs/`. Root giờ chỉ còn README + CREDITS (đúng quy ước).
- **2 link README gãy** (`artstyle.md`, `BUILD.md` — đã dời vào docs/ từ lúc merge) → sửa.
- `CODEMAP.md` cập nhật mục block_factory → block_catalog.

### `docs/MAINTAIN.md` — sổ tay cho người không rành code (tiếng Việt)
